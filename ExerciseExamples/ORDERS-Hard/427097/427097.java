...
class AVLTree
{
	...
	node find(int val)
	{
		...
		while(!curr.isLeaf)
		{
			int c = new Integer(val).compareTo(curr.data);
			if(c == 0)
				return curr;	//found
			else if(c < 0)
			{
	...
	node add(int val)
	{
		...
		while(!curr.isLeaf)
		{
			c = new Integer(val).compareTo(curr.data);
			if(c < 0)
			{
				if(!curr.left.isLeaf)
					curr = curr.left;
	...
	int findAtRank(int rank,int N)	//return value at 0-indexed sorted list of numbers
	{
		int ret = 0;
		node curr = root;
		int c;
...
public class Main
{
	...
	void solve() throws IOException
	{
		...
		for(int i=0;i<=N;i++)
		{
			debug(i,tree.lowerboundRank(i),tree.upperboundRank(i),tree.findAtRank(i,N));
		}
		...
		*/
		...
		for(int i=N-1;i>=0;i--)
		{
			int z=tree.findAtRank(i-shift[i],N);
			ranks[i]=z;
			tree.del(z);
		}
