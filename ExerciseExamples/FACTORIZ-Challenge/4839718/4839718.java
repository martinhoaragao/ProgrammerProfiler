...
class FactorizationBrute_2 {
    ...
    public static final BigInteger LongMaxValue = new BigInteger(
            "10000000000000000");
        ...
        try {
            ...
            for (int t = 0; t < T; t++) {
                ...
                if (bigN.compareTo(LongMaxValue) < 0) {
                    ...
                    bigDivisorsList = processBigN(bigN);
    ...
    public static BigInteger[] bigPrimeList;

    public static void initBigPrimeList() {
        bigPrimeList = new BigInteger[primeList.length/1000];

        for (int i = 0; i < primeList.length/1000; i++) {
            bigPrimeList[i] = new BigInteger(Integer.toString(primeList[i]));
        }
    }
    ...
    public static ArrayList<BigInteger> processBigN(BigInteger bigN) {
        ...
        int i = 0;
        BigInteger bigPL;
        bigPL = bigPrimeList[i];
        for (; bigPL.compareTo(bigN) < 0 && i < bigPrimeList.length - 2;) {
            // Check divisibility
//            System.out.println(i + "[] = " + bigPL + "&& bigN" + bigN);
            if ((!BigInteger.ZERO.equals(bigPL)) && bigN.divide(bigPL).multiply(bigPL).equals(bigN)) {
                result.add(bigPL);
                bigN = bigN.divide(bigPL);
                i = 0;
            } else {
                i++;
            }
