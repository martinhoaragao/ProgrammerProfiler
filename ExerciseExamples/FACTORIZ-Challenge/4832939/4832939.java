...
public class Main {
    ...
    static double[] pLog = new double[1000005];
    static int prCnt = 0;
    ...
    public static void main(String[] args) {
        ...
        for (long i = 2; i < 300000; i++) {
            if (pm[(int) i])
                continue;
            primes2[prCnt++] = i;
            pLog[prCnt - 1] = Math.log10(primes2[prCnt - 1]);
    ...
    public static void solve(BufferedReader in, PrintWriter out) throws IOException {
        ...
        for (int i = 0; i < 25; i++) {
            int t = cases[i].x.bitLength();

            cases[i].solve(450, 14000);
            //cases[i].solve(10000, 20000);
        }
    ...
    public static class TestCase {
        ...
        public void solve(int lowerBound, int upperBound) {
            double len = Math.max(18.0, x.bitLength() / 30.0);
            for (int i = lowerBound; i < upperBound; i++) {
                ...
                while (j < upperBound) {
                    ...
                    if (sumLog < Math.max(18.0, x.bitLength() / 16.0)) {
